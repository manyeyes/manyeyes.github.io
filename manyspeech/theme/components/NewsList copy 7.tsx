...
import { useLang, useLocation } from '@rspress/core/runtime';   // 导入 useLocation
...
const ArticleList = () => {
  const i18n = useI18n();
  const lang = useLang();
  const { pathname } = useLocation();  // 监听路由变化
  const [posts, setPosts] = useState<Post[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const base = import.meta.env.BASE_URL || '/';
    // 构造多语言 JSON 路径：中文无前缀，其他语言添加语言前缀
    // const langPath = lang === 'zh' ? '' : `${lang}/`;
    const langPath = lang;
    // 去除多余斜杠，例如 /manyspeech//blog/ -> /manyspeech/blog/
  ...
  }, [lang, pathname]);  // 同时依赖 lang 和 pathname，确保语言切换时刷新
